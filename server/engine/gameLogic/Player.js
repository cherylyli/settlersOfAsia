 ...
 * @return {Number}
 */
Player.getBuildingCnt = function (player) {
...
Player.drawRandomFishNoBoot = function(player){
...
Player.giveAwayBoot = function (player, opponentPlayer){
...
Player.spendFishToken = function(player, action, match){
    let newSum = 0;
    switch(action){
        case "MOVE_ROBBER" :
 ...
 */
Player.drawOneProgressCard = function(player, match, kind){
    var progCardList = [];
    var playersCards = [];
 ...
 * @return card {String}
 */
Player.stolenBy = function (player, opponentPlayer) {
    if (Player.resourceCardTotalNum(player) < 1){
...
Player.stealCard = function (player, opponentPlayer) {
    let keys = [];
 ...
 */
Player.updateVP = function(player, points){
    player.VP += points;
    Match.checkPlayerVP(player.match, player);
}; //HERE
 ...
 * @return {Building}
 */
Player.getBuilding = function (player, vertex) {
...
Player.getCities = function (player) {
    let cities = [];
 ...
 *
 */
Player.getSettlements = function (player) {
    let settlements = [];
    for (let vertex in player.buildings){
 ...
 *
 */
Player.progCardSum = function(player){
    player.progressCardsCnt = player.progressCards.length;
...
Player.discardOneProgressCard = function(player, card){
...
Player.calculateLongestRoad = function(player, map){
...
Player.discardedCardsCnt = function(player){
    var numToBeDiscarded = -1;
    if(Player.resourceCardTotalNum(player) <= player.maxSafeCardNum){
        return 0;
    }
    else {
        ...
        numToBeDiscarded = Math.floor(Player.resourceCardTotalNum(player)/2) - cardsToKeep;
    }
...
Player.discardResourceCards = function(player, cards){
    let keys = [];
...
Player.getKnightsSum = function(player){
    var sum = 0;
...
Player.offerTrade = function(player, offer,receiver){
    //TODO

};

Player.acceptTrade = function(player, offer,willAccept){
    //TODO

};

Player.setDefenderOfCatan = function(player, result){
    player.defenderOfCatan = result;
};

Player.getDefenderOfCatan = function(player){
